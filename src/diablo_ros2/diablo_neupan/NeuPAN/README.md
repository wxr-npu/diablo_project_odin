...
This is a modified version of NeuPAN. For the original repository, please refer to [here](https://github.com/hanruihua/NeuPAN).
...
git clone --depth 1 --recursive https://github.com/XXX/NeuPAN
cd NeuPAN
conda install -c conda-forge mamba
mamba create -n neupan -y
mamba activate neupan
mamba config --env --add channels conda-forge
mamba config --env --remove channels defaults
mamba config --env --add channels robostack-${ROS_DISTRO}
...
``` shell
mamba activate neupan
python neupan/ros/neupan_ros.py
```
...
- Currently, NeuPAN supports the kinematics of differential drive robots and Ackermann robots (see the `robot` section in the YAML file). For robots with other kinematics, you can modify the constraints in the NRMP layer to suit your needs.
