...
class robot:
    ...
    def C0_cost(self, para_p_u, para_q_s):
        ...
        para_q_s: weight of state cost for x, y
        para_q_s2: weight of state cost for theta
        '''
        # use for theta
        para_q_s2 = 1.0
        diff_u = para_p_u * self.indep_u[0, :] - self.para_gamma_b
        diff_s_xy = para_q_s * self.indep_s[0:2, :] - self.para_gamma_a[0:2, :]
        diff_s_th = para_q_s2 * self.indep_s[2:3, :] - self.para_gamma_a[2:3, :]

        C0_cost = cp.sum_squares(diff_s_xy) + cp.sum_squares(diff_s_th) + cp.sum_squares(diff_u) 

        return C0_cost

    def proximal_cost(self, bk_linear: float, bk_theta: float):

        """
        bk_linear: float -- weight for linear proximal cost
        bk_theta: float -- weight for theta proximal cost
        proximal cost
        """

        proximal_cost = bk_linear * cp.sum_squares(self.indep_s[0:2, :] - self.para_s[0:2, :]) + bk_theta * cp.sum_squares(self.indep_s[2:3, :] - self.para_s[2:3, :])
