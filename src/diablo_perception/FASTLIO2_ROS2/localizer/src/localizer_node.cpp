...
struct NodeState
{
    ...
    CloudType::Ptr last_cloud = boost::make_shared<CloudType>();
...
public:
    ...
    void loadGlobalPcdMap()
    {
        RCLCPP_INFO(this->get_logger(), "Loading map ............................." );
        std::string pcd_file;
        this->declare_parameter("pcd_file", "");
        pcd_file = this->get_parameter("pcd_file").as_string();

        if (!std::filesystem::exists(pcd_file))
        {
            RCLCPP_ERROR_STREAM(this->get_logger(), "pcd file not found");
            return;
        }
        RCLCPP_INFO_STREAM(this->get_logger(), "pcd file: " << pcd_file);
        bool load_flag = m_localizer->loadMap(pcd_file);
        RCLCPP_INFO(this->get_logger(), "load_flag: %s", load_flag ? "true" : "false" );
        if (!load_flag)
        {
            RCLCPP_ERROR(this->get_logger(), "Load map failed");
            return;
        } else { 
            RCLCPP_INFO_STREAM(this->get_logger(), "Load map from : "<< pcd_file);
        }
        m_state.global_map_received = true;
    }
    void timerCB()
    {
        if (!m_state.global_map_received) {
            builtin_interfaces::msg::Time load_map_stamp;
            load_map_stamp = this->get_clock()->now();
            publishMapCloud(load_map_stamp);
            return;
        }
        ...
        if (result)
        {
            RCLCPP_INFO(this->get_logger(), "Localizer aligned");
        ...
        {
            RCLCPP_INFO(this->get_logger(), "Localizer failed to align");
        }
    ...
    void relocCB(const std::shared_ptr<interface::srv::Relocalize::Request> request, std::shared_ptr<interface::srv::Relocalize::Response> response)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "Set Initial guess..............................");
