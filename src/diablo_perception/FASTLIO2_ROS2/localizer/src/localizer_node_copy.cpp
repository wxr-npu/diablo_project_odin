...
public:
    ...
    void relocCB(const std::shared_ptr<interface::srv::Relocalize::Request> request, std::shared_ptr<interface::srv::Relocalize::Response> response)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "PCD_RECEIVED-1: %s", PCD_RECEIVED ? "true" : "false");
        if(!PCD_RECEIVED && !pcd_path.empty()) {
            ...
            if (!load_flag)
            {
                ...
                response->message = "load map success, please click pose in rviz";
                PCD_RECEIVED = true;
                return;
            }
        } 
        if (pcd_path.empty())
        {
            RCLCPP_INFO(this->get_logger(), "PCD_RECEIVED-2: %s", PCD_RECEIVED ? "true" : "false");
            {
                std::lock_guard<std::mutex>(m_state.message_mutex);
