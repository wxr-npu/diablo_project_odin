...
nav2_recoveries::Status BackUpFreeSpace::onRun(
...
nav2_recoveries::Status BackUpFreeSpace::onCycleUpdate()
{
  ...
  geometry_msgs::msg::Pose2D pose, float radius, float first_safe_angle, float last_unsafe_angle)
{
  ...
  sector_marker.header.stamp = steady_clock_.now();
  ...
  auto create_arrow = [&](float angle, int id, float r, float g, float b) {
    ...
    arrow.header.stamp = steady_clock_.now();
    arrow.ns = "direction";
    arrow.id = id;
...
PLUGINLIB_EXPORT_CLASS(pb_nav2_recoveries::BackUpFreeSpace, nav2_core::Recovery)
