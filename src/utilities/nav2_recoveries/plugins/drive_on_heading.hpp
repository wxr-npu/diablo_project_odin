...
#ifndef nav2_recoveries__PLUGINS__DRIVE_ON_HEADING_HPP_
#define nav2_recoveries__PLUGINS__DRIVE_ON_HEADING_HPP_
...
#include "nav2_preview_msgs/action/drive_on_heading.hpp"
#include "nav2_preview_msgs/action/back_up.hpp"
...
/**
 * @class nav2_recoveries::DriveOnHeading
 * @brief An action server Behavior for spinning in
 */
template<typename ActionT = nav2_preview_msgs::action::DriveOnHeading>
...
public:
  /**
   * @brief A constructor for nav2_recoveries::DriveOnHeading
   */
  DriveOnHeading()
  : Recovery<ActionT>(),
  ...
  Status onCycleUpdate() override
  {
    ...
    cmd_vel->header.stamp = this->steady_clock_.now();
    ...
    auto cmd_vel_no_stamp = geometry_msgs::msg::Twist();
    cmd_vel_no_stamp = cmd_vel->twist;
    this->vel_pub_->publish(std::move(cmd_vel_no_stamp));
...
protected:
  ...
  void onConfigure() override
  {
    ...
    nav2_util::declare_parameter_if_not_declared(
      node, this->recovery_name_ + ".acceleration_limit",
      rclcpp::ParameterValue(2.5));
    nav2_util::declare_parameter_if_not_declared(
      node, this->recovery_name_ + ".deceleration_limit",
      rclcpp::ParameterValue(-2.5));
    nav2_util::declare_parameter_if_not_declared(
      node, this->recovery_name_ + ".minimum_speed",
      rclcpp::ParameterValue(0.10));
    node->get_parameter(this->recovery_name_ + ".acceleration_limit", acceleration_limit_);
    node->get_parameter(this->recovery_name_ + ".deceleration_limit", deceleration_limit_);
    node->get_parameter(this->recovery_name_ + ".minimum_speed", minimum_speed_);
...
#endif  // nav2_recoveries__PLUGINS__DRIVE_ON_HEADING_HPP_